...
class Present(db.Model):
    ...
    _from = db.Column(db.Integer, db.ForeignKey('friend.friend_id', ondelete='CASCADE'), unique=False, nullable=False)
    to = db.Column(db.Integer, db.ForeignKey('client.client_id', ondelete='CASCADE'), unique=False, nullable=False)
